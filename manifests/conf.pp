...
#   Example: source => 'puppet:///site/redis/my.conf',
#
# [*ensure*]
...
define redis::conf (
  ...
  $managed_mode = $mode ? {
    undef   => $redis::file_mode,
    default => $mode,
  }

  $managed_owner = $owner ? {
    undef   => $redis::file_owner,
    default => $owner,
  }

  $managed_group = $group ? {
    undef   => $redis::file_group,
    default => $group,
  }

  $managed_require = $require ? {
    undef   => $redis::file_require,
    default => $require,
  }

  $managed_notify = $notify ? {
    undef   => $redis::file_notify,
    default => $notify,
  }

  $managed_replace = $replace ? {
    undef   => $redis::file_replace,
    default => $replace,
  }
